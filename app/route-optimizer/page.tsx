...
export default function RouteOptimizer() {
  ...
  const [selectedIndustry, setSelectedIndustry] = useState('cleaning'); // Default to existing cleaning data
  ...
  const availableIndustries = [
    { id: 'cleaning', name: '🧽 Cleaning', table: 'jobs' }, // Your existing data
    { id: 'landscaping', name: '🌱 Landscaping', table: 'service_jobs' }, // New data
    { id: 'electrical', name: '⚡ Electrical', table: 'service_jobs' },
    { id: 'plumbing', name: '🔧 Plumbing', table: 'service_jobs' }
  ];
  ...
  // Enhanced fetch jobs function with multi-industry support
  const fetchJobs = async () => {
    setJobsLoading(true);
    try {
      const supabase = createClient();
      
      // Get the correct table based on selected industry
      const currentIndustry = availableIndustries.find(ind => ind.id === selectedIndustry);
      const tableName = currentIndustry?.table || 'jobs';
      
      let query = supabase.from(tableName).select('*');
      
      // Add industry filter for multi-industry tables
      ...
      const transformedJobs: JobLocation[] = data.map((job) => {
        if (tableName === 'service_jobs') {
          // Handle new landscaping/multi-industry data structure
          return {
            id: job.id.toString(),
            ...
            status: 'pending', // All new multi-industry jobs start as pending
            scheduledTime: '09:00', // Default time - can be enhanced later
            driveTo: {
          ...
          // Handle existing cleaning data structure (your current logic)
          return {
            id: job.id,
            customerName: job.customer_name || 'Unknown Customer',
            address: job.address || 'Address not specified',
      ...
      console.log(`✅ Successfully loaded ${transformedJobs.length} ${selectedIndustry} jobs from ${tableName}`);
  ...
  // Update job status in appropriate table
  ...
  const optimizeRouteWithMaps = useCallback(async () => {
    ...
    try {
      ...
      if (mockResult.success) {
        ...
        alert(`Route optimized successfully with real ${selectedIndustry} job data!`);
      }
    } catch (error) {
  ...
  }, [jobs, selectedDate, selectedCrew, selectedIndustry]);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-white/80 mt-2">
              {jobs.length > 0 ? `Managing ${jobs.length} ${selectedIndustry} jobs (today)` : `No ${selectedIndustry} jobs found - add some jobs to get started!`}
            </p>
          </div>
          <button
            onClick={fetchJobs}
        ...
        {jobs.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h2 className="text-2xl font-bold text-white mb-4">No {selectedIndustry} Jobs to Optimize</h2>
            <p className="text-white/80 mb-6">Add some {selectedIndustry} jobs to your dashboard to start route optimization</p>
            <button
          ...
          <>
            {/* Enhanced Controls with Industry Selector */}
            <div className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                <label htmlFor="selectedIndustry" className="block text-sm font-semibold text-white/80 mb-2">Industry</label>
                <select
                  id="selectedIndustry"
                ...
                >
                  {availableIndustries.map(industry => (
                    <option key={industry.id} value={industry.id} className="bg-blue-800 text-white">
                      {industry.name}
                    </option>
                  ))}
                </select>
                  ...
                  <div className="relative h-96 bg-gray-200 rounded-lg overflow-hidden border-2 border-white/30">
                    ...
                    <div className="absolute top-4 left-4 bg-white rounded-lg shadow-xl p-3">
                      <h4 className="font-bold text-gray-800 text-sm flex items-center">
                        ...
                        {selectedIndustry === 'landscaping' ? 'Loudoun County, VA' : 'Baltimore, MD'}
                      </h4>
                  ...
                  <div className="space-y-4">
                    <div className="bg-white/10 p-4 rounded-lg border border-white/20">
                      ...
                      <p className="text-white/80 text-sm">
                        {routeStats ? `${routeStats.efficiency}% efficiency with ${routeStats.totalJobs} ${selectedIndustry} jobs` : 'Waiting for route data...'}
                      </p>
                      <p className="text-green-400 text-xs mt-1">Multi-industry data from Supabase</p>
                    </div>
                    ...
                    <div className="bg-white/10 p-4 rounded-lg border border-white/20">
                      ...
                      <p className="text-blue-400 text-xs mt-1">Live updates from {selectedIndustry} database</p>
                    </div>
                    ...
                    <div className="bg-white/10 p-4 rounded-lg border border-white/20">
                      ...
                      <p className="text-white/80 text-sm">
                        {routeStats ? formatDuration(routeStats.totalWorkTime) : '0m'} estimated across all {selectedIndustry} jobs
                      </p>
                    ...
                    <div className="bg-white/10 p-4 rounded-lg border border-white/20">
                      ...
                      <p className="text-white/80 text-sm">
                        {selectedIndustry === 'landscaping' 
                          ? 'Ashburn & Leesburg area focus' 
                          : `Jobs across ${new Set(jobs.map(j => j.city).filter(Boolean)).size} cities`
                        }
                      </p>
                      <p className="text-purple-400 text-xs mt-1">
                        {selectedIndustry === 'landscaping' ? 'Loudoun County, VA' : 'Baltimore metro area'}
                      </p>
                    </div>
                  </div>
                </div>
